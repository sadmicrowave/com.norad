...
var  config		  = require('../../config.js')
	//,sendgrid     = require('sendgrid')(config.sendgrid_apikey)
...
function ActivationEmail(activation_key, useremail, callback, router) {
    sendgrid.setApiKey(config.sendgrid_apikey)
    // Activation email sends to user with activationUrl link in active now button
	sendgrid.send({
			 'to': useremail
			,'from':'Activation@norad.com'
			,'subject':'Welcome To Norad! Confirm Your Email'
			,'text':' '
			,'html':' '
			,'templateId' : '388f5b29-d572-4b8a-98fd-80856ed452a0' // sendgrid template id for activation email template
			,'substitutionWrappers': ['{{', '}}']
			,'substitutions': { 'activation_url': 'http://{0}:{1}/users?ak={2}'.format(process.env.FQDN, process.env.PORT, activation_key)
							  }
	}, (err, result) => {
...
function ContactEmail(useremail, message, callback, router) {
	...
	sendgrid.send({
			...
			,'from':'contact@norad.com'
			,'subject':'Norad Contact Message'
			,'text':' '
			,'html':' '
			,'templateId' : 'd0b76ddb-930a-4169-96d4-3882647d71cd' // sendgrid template id for activation email template
			,'substitutionWrappers': ['{{', '}}']
			,'substitutions': {  'sending_user_email': useremail
