...
// ****************************************
// -------- Log into Access log every req
// ****************************************
Router.prototype.access = function(req, res, body, callback){
	this.res = res;
	this.req = req;
	req.router = this;
	...
	var  iat	= new Date().getTime() 	// epoch time at which call was initiated
		,ip = getClientAddress(req)
		;

	MongoClient.connect('mongodb://localhost:27017/norad', function(err, db){
		if (err) return req.router.error( Error(err) );
		db.collection('access', function(err, coll){
					...
					coll.insert({ 	 'ip'		: ip
								...
								,function(err, res){
									console.log( err );
									if ( err ) {
										return req.router.error( Error(err) );
									} else {
										console.log("made it here:", res );
										// execute user passed callback after authenticate and request increment queries are performed
										if ( callback instanceof Function ) return callback(req, res, body);
									}
								}
					); // close coll.insert
		}); // close db.collections('access', ... )
						...
						var dt			= new Date().getTime()
							,updateSet 	= { 'total_req'		: isNaN(doc.total_req) ? 0 : doc.total_req + 1 //increment total request counter by one
				 							,'modified'	: dt // set modified date to current datetime stamp
										  	...
										  	// if difference between current timestamp and start timestamp in minutes is greater than 24 hours
											.extend( parseFloat(((dt - doc.threshold_start_time)/1000/60).toFixed(2)) > 1442 
			...
			coll.insert({  	 'url'					: 	req.url
						...
						function(err){
							...
							var  	 url 		= urlapi.parse(req.url)
									...
									,match		= params.jwt ? req.test(params.jwt) : null
							...
							req.extend({'token': token, 'path': url.path, 'params': Object.keys(params).length ? params : querystring.parse(decodeURIComponent( body )) }); //, 'data': body.length ? querystring.parse(decodeURIComponent( body )) : { 'option': option } })
